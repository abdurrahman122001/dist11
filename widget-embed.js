...
(function () {
  ...
  // Müəyyən et ki, widget-embed.js haradan yüklənib (Frontend URL)
  var scriptOrigin = "";
  var scripts = document.getElementsByTagName("script");
  for (var i = 0; i < scripts.length; i++) {
    var src = scripts[i].src || "";
    if (src.indexOf("widget-embed.js") !== -1) {
      scriptOrigin = new URL(src).origin;
      break;
    }
  }

  // Əgər apiUrl verilməyibsə, script-in gəldiyi yeri istifadə et
  var apiUrl = (cfg.apiUrl || scriptOrigin || "").replace(/\/$/, "");

  // iframe-i həmişə FRONTEND-dən (scriptOrigin) yüklə
  var embedSrc = (scriptOrigin || window.location.origin) + "/embed.html?apiKey=" + encodeURIComponent(apiKey) + "&apiUrl=" + encodeURIComponent(apiUrl);
